...
class HomeViewModel: ViewModel() {
    ...
    private val _recentList = MutableLiveData<List<DataItem>>()
    val recentList: LiveData<List<DataItem>> get() = _recentList
    ...
    fun init() {
        ...
        user?.let {
            ...
            userRepository.getUserTopics(it.uid, null, 5).observeForever { result ->
                val dataList = result.first
                _recentList.postValue(dataList)
            }
    ...
    fun loadMoreData() {
        ...
        {
            _loadingIndicator.value = true
            topicRepository.getNewTopics(startAfterDocument,100).observeForever {result ->
                val topics = result.first
                hasMoreData = result.second.first
                startAfterDocument = result.second.second
